...
unicorn_config "#{node['unicorn_config_path']}/#{app_name}.rb" do
  preload_app true
  listen("#{node[app_name]['paths']['shared']}/tmp/sockets/unicorn.socket" => {backlog: 1024})
  pid("#{node[app_name]['paths']['shared']}/tmp/pids/unicorn.pid")
  stderr_path("#{node[app_name]['paths']['shared']}/log/unicorn.stderr.log")
  stdout_path("#{node[app_name]['paths']['shared']}/log/unicorn.stdout.log")
  worker_timeout 60
  worker_processes [node['cpu']['total'].to_i * 4, 8].min
  working_directory "#{node[app_name]['paths']['deploy']}"
